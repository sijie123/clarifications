  ...
  return (
    <Table striped bordered hover>
      ...
      <tbody>
        {
          ans.map(threadID => {
            return <tr><td className="expand"><a href="#" onClick={e => setCurrentlyFocusedThreadID(threadID)}>{clarificationData.threads[threadID].title}</a></td><td className="shrink">{clarificationData.threads[threadID].answer}</td><td className="shrink">{clarificationData.threads[threadID].contestantid}</td></tr>
          })
        }
      </tbody>
    </Table>
